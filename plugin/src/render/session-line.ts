...
import { coloredBar, critical, git as gitColor, gitBranch as gitBranchColor, label, model as modelColor, project as projectColor, effort as effortColor, getContextColor, getQuotaColor, quotaBar, custom as customColor, RESET } from './colors.js';
...
export function renderSessionLine(ctx: RenderContext): string {
  ...
  const effortPart = ctx.effortLevel ? ` ${effortColor(ctx.effortLevel, colors)}` : '';
  if (display?.showModel !== false && display?.showContextBar !== false) {
    parts.push(`${modelColor(`[${modelDisplay}]`, colors)}${effortPart} ${bar} ${contextValueDisplay}`);
  } else if (display?.showModel !== false) {
    parts.push(`${modelColor(`[${modelDisplay}]`, colors)}${effortPart} ${contextValueDisplay}`);
  ...
  const customLine = display?.customLine
    ? display.customLine.replace(/\{username\}/g, os.userInfo().username)
    : undefined;
  if (customLine) {
