...
export function renderProjectLine(ctx: RenderContext): string | null {
  ...
  const customLine = display?.customLine
    ...
    : undefined;
  if (customLine) {
    parts.push(customColor(`${osInfo} │ ${customLine}`, colors));
  } else {
  ...
  if (showGit && ctx.gitStatus) {
    ...
    if (gitConfig?.showFileStats && ctx.gitStatus.fileStats) {
      ...
      if (modified > 0) statParts.push(yellow(`!${modified}`));
      if (added > 0) statParts.push(brightGreen(`+${added}`));
      if (deleted > 0) statParts.push(red(`-${deleted}`));
